...
#include <test_baxter_mover_utils/object_position.h>
...
class baxter_controller{
    ...
    public :
        ...
        void init(){
                ...
                _nh.getParam("/dream_babbling/params", glob_params);
                ...
                ROS_WARN_STREAM("CONTROLLER : Trying to initialize the robot mover class, name space is: " << _nh.getNamespace());
                _baxter_mover.reset(new baxter_mover::BAXTER_Mover(_nh));
                _move_baxter_arm.reset(new ros::ServiceClient(_nh.serviceClient<baxter_mover_utils::move_baxter_arm>("/move_baxter_arm")));
                _sub_joint_states.reset(new ros::Subscriber(_nh.subscribe("/joint_states",1,&baxter_controller::jostateCallback, this)));
                _tf_subscriber.reset(new ros::Subscriber(_nh.subscribe("/tf", 10, &baxter_controller::poseCallback, this)));
                _object_position_subscriber.reset(new ros::Subscriber(_nh.subscribe("/object_position", 1, &baxter_controller::execute, this)));
                _object_touched_pub.reset(new ros::Publisher(_nh.advertise<std_msgs::Bool>("/object_touched", 1)));
        ...
        void execute(const test_baxter_mover_utils::object_position::ConstPtr& poseGoal)
            {
                ROS_INFO_STREAM("CONTROLLER : Received new goal, I am planning for X = " << poseGoal->object_position[0]
                        << ", Y = " << poseGoal->object_position[1]
                        << " and Z = " << poseGoal->object_position[2]);
                _goal = poseGoal->object_position;
                if(_first_goal){
                        _first_goal = false;
                ...
                while(!complete_plan && iteration < _number_of_trials){
                        ...
                        while((vectors_distance(_goal, vector_1) - _extension_distance) > 0.01){
                                _target = get_random_targets_around_sphere(_number_of_points, poseGoal->object_position);
                        ...
                        //                        else
                        //                            _arm = "right_arm";
                        if(poseGoal->object_position[1] >= 0)
                            _arm = "left_arm";
                        else
                            _arm = "right_arm";
                        ...
                        if(plan_result){
                                ...
                                if(fraction >= _straight_line_threshold){
                                        ...
                                        ROS_INFO_STREAM("CONTROLLER : _touch_object_success is : " << _touch_object_success);
                        ...
                        ROS_INFO_STREAM("CONTROLLER : second place : _touch_object_success is : " << _touch_object_success);
                        iteration++;
                    }
                if(reversed)
                    octomap_manipulation(true);
                ...
                if(_touch_object_success){
                        ...
                        //publish true in a topic called object_touched
                        _object_touched_msg.data = true;
                        _object_touched_pub->publish(_object_touched_msg);
                    }
                else{
                        _failed_iteration++;
                        _total_iteration++;
                        ROS_WARN_STREAM("CONTROLLER : Something went wrong");
                        //publish false in a topic called object_touched
                        _object_touched_msg.data = false;
                        _object_touched_pub->publish(_object_touched_msg);
                    }
    ...
    private:
        ros::NodeHandle _nh;
        std::shared_ptr<ros::Publisher> _scene_publisher, _object_touched_pub;
        std::shared_ptr<ros::Subscriber> _sub_joint_states, _tf_subscriber, _object_position_subscriber;
        ...
        std_msgs::Bool _object_touched_msg;
...
int main(int argc, char** argv){
        ros::init(argc, argv, "baxter_mover_node");
        ros::NodeHandle outer_handler;

        baxter_controller my_baxter_controller;

        ROS_INFO_STREAM("CONTROLLER : Robot controller ready !");

        ros::Rate the_rate(50);
